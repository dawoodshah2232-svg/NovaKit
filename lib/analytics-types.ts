export interface AnalyticsDashboardData {
  ...
  metrics: { online: number; visitors: number; sessions: number; views: number; tools: number; duration: number; allTimeVisitors: number; todayVisitors: number; yesterdayVisitors: number; thisMonthVisitors: number; previousMonthVisitors: number; newVisitors: number; returningVisitors: number };
  ...
  history: VisitorHistoryRow[];
  historyMeta: { page: number; pageSize: number; total: number; totalPages: number };
}
export interface Breakdown { name: string; value: number }
export interface VisitorHistoryRow {
  session_id: string; first_seen: string; landing_path: string; last_path: string;
  country: string | null; device: string; browser: string; source: string;
  duration: number; page_views: number; tool_uses: number;
}
