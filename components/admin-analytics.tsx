...
export function AdminDashboard() {
  ...
  const cards = data ? [ ['Online Now', data.metrics.online], ['All-time Visitors', data.metrics.allTimeVisitors], ['Today Visitors', data.metrics.todayVisitors], ['Yesterday Visitors', data.metrics.yesterdayVisitors], ['This Month Visitors', data.metrics.thisMonthVisitors], ['Previous Month Visitors', data.metrics.previousMonthVisitors], ['New Visitors', data.metrics.newVisitors], ['Returning Visitors', data.metrics.returningVisitors], ['Total Sessions', data.metrics.sessions], ['Total Page Views', data.metrics.views], ['Total Tool Runs', data.metrics.tools], ['Avg Session Duration', duration(data.metrics.duration)] ] : [];
  const setDashboardRange = (value: string) => { setData(null); setError(''); setHistoryPage(1); setRange(value); };
  ...
  return <div className="min-h-screen rounded-3xl bg-slate-950 p-4 text-slate-100 sm:p-7">
    ...
    <div className="mb-5 space-y-3"><div className="flex flex-wrap items-center justify-between gap-3"><div role="group" aria-label="Analytics time range" className="flex flex-wrap gap-1 rounded-xl border border-slate-800 p-1">{ranges.map(([value,label]) => <button key={value} aria-pressed={range === value} onClick={() => setDashboardRange(value)} className={`rounded-lg px-3 py-2 text-sm ${range === value ? 'bg-cyan-400 text-slate-950' : 'text-slate-400 hover:text-white'}`}>{label}</button>)}</div><p className="text-xs text-slate-400" aria-live="polite">{data ? `Last updated ${age} seconds ago${error ? ' · stale' : ''}` : 'Waiting for database'}</p></div>
    ...
    {data && <div className="space-y-6">
      ...
      <p className="text-xs text-slate-500">Range totals and charts follow the selected Dubai-time window. Visitors are distinct anonymous session IDs, not identified people. Duration measures observed active time for the selected visitors.</p>
      ...
      <Section title="Visitor History"><div className="mb-4 flex flex-wrap items-end gap-2"><label className="text-xs text-slate-400">Country<select value={country} onChange={event => { setCountry(event.target.value); setHistoryPage(1); }} className="mt-1 block rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white"><option value="">All countries</option>{data.geography.map(row => <option key={row.name} value={row.name}>{row.name}</option>)}</select></label><label className="text-xs text-slate-400">Device<select value={device} onChange={event => { setDevice(event.target.value); setHistoryPage(1); }} className="mt-1 block rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white"><option value="">All devices</option>{data.devices.map(row => <option key={row.name} value={row.name}>{row.name}</option>)}</select></label><label className="text-xs text-slate-400">Source<select value={source} onChange={event => { setSource(event.target.value); setHistoryPage(1); }} className="mt-1 block rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white"><option value="">All sources</option>{data.acquisition.map(row => <option key={row.name} value={row.name}>{row.name}</option>)}</select></label><label className="text-xs text-slate-400">Page<input value={pageFilter} onChange={event => { setPageFilter(event.target.value); setHistoryPage(1); }} placeholder="/merge-pdf" className="mt-1 block w-36 rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white" /></label><button className={button} onClick={clearTableFilters}>Clear filters</button></div>{data.history.length ? <div className="overflow-x-auto"><table className="w-full min-w-[980px] whitespace-nowrap text-left text-xs"><thead className="text-slate-500"><tr>{['Time','Anonymous session','Landing page','Last/current page','Country','Device/browser','Source/referrer','Duration','Page views','Tool uses'].map(label => <th key={label} className="px-3 pb-3 font-medium">{label}</th>)}</tr></thead><tbody>{data.history.map(row => <tr key={row.session_id} className="border-t border-slate-800"><td className="px-3 py-4">{time(row.first_seen)}</td><td className="px-3 font-mono">{row.session_id.slice(0,8)}</td><td className="px-3">{row.landing_path}</td><td className="px-3">{row.last_path}</td><td className="px-3">{row.country || 'Unknown'}</td><td className="px-3">{row.device} / {row.browser}</td><td className="px-3">{row.source}</td><td className="px-3">{duration(row.duration)}</td><td className="px-3">{row.page_views}</td><td className="px-3">{row.tool_uses}</td></tr>)}</tbody></table></div> : empty}<div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400"><span>{data.historyMeta.total.toLocaleString()} sessions · Page {data.historyMeta.page} of {data.historyMeta.totalPages}</span><div className="flex gap-2"><button className={button} disabled={data.historyMeta.page <= 1} onClick={() => setHistoryPage(value => value - 1)}>Previous</button><button className={button} disabled={data.historyMeta.page >= data.historyMeta.totalPages} onClick={() => setHistoryPage(value => value + 1)}>Next</button></div></div></Section>
