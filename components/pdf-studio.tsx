...
export function PdfStudio() {
  ...
  const loadPdfData = useCallback(async (data: Uint8Array, fileName: string) => {
    ...
    try {
      ...
      trackToolExecution('pdf_studio', 'load_file', numPages);
    } catch (err: any) {
      console.error('Failed to load PDF in studio:', err);
      setErrorMsg(err.message || 'Unable to load PDF document. Please verify the file is a valid, unencrypted PDF.');
    } finally {
      setBusy(false);
      setStatusMsg('');
    }
  ...
  const loadSamplePdf = async () => {
    ...
    try {
      ...
      setErrorMsg('Failed to generate sample PDF: ' + err.message);
      setBusy(false);
    }
  };
  ...
  const exportModifiedPdf = async (options: { flatten?: boolean } = {}) => {
    ...
    try {
      ...
      trackToolExecution('pdf_studio', 'export_pdf', pages.length);
    } catch (err: any) {
  ...
  const extractTextFromActiveDoc = async () => {
    ...
    try {
      ...
      trackToolExecution('pdf_studio', 'extract_text', pdf.numPages);
    } catch (err: any) {
  ...
  const exportImagesZip = async () => {
    ...
    try {
      ...
      trackToolExecution('pdf_studio', 'export_images', pdf.numPages);
    } catch (err: any) {
      setErrorMsg('Failed to export images ZIP: ' + err.message);
    } finally {
      setBusy(false);
      setStatusMsg('');
    }
          ...
          <main className="flex-1 bg-slate-900 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-auto p-4 sm:p-8 flex items-center justify-center bg-slate-900/50">
              ...
              >
                ...
                {isDraggingRedact && redactStart && redactCurrent && mainCanvasContainerRef.current && (
                  <div
              ...
              {activeTab === 'sign' && (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      value={sigPosition}
                      onChange={(e: any) => setSigPosition(e.target.value)}
                      className="w-full px-2.5 py-1.5 bg-slate-900 border border-slate-800 rounded-lg text-xs text-white"
                    >
