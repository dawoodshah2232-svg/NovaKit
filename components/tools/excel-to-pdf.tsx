...
export function ExcelToPdf() {
  ...
  const handleFile = async (selectedFile: File) => {
    ...
    try {
      ...
      setErrorMsg((err instanceof Error && err.message) || 'Failed to read spreadsheet file. Please check file format.');
      setSheetData(null);
...
Q3 2026,AI Workspace,Asia Pacific,24500,"$2,450,000",+26.9%,Projected`;
  ...
  const generatePdf = () => {
    ...
    try {
      ...
      setErrorMsg('Failed to generate PDF: ' + (err instanceof Error ? err.message : 'unknown error'));
      trackToolExecution('excel-to-pdf', false);
    } finally {
      setBusy(false);
      setStatusMsg('');
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <select
                  value={themeColor}
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => setThemeColor(e.target.value as 'blue' | 'indigo' | 'emerald' | 'slate' | 'rose')}
